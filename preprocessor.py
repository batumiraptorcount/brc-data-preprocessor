...
data_csv = 'data/2018.csv'
season_start = '2018-08-17'
season_end = '2018-10-16'
hb_focus_start = '2018-08-21'
hb_focus_end = '2018-09-09'
window_minutes = 5  # window used to check total number of birds with aged numbers
...
data.drop(columns=['date', 'timestamp', 'countid', 'speciesid', 'year', 'yday'])

# Change column order
column_order = ['datetime', 'telpost', 'speciesname', 'count', 'countback', 'local', 'age', 'sex', 'plumage', 'remark',
                'location', 'migtype', 'counttype']
...
# Remove all records from counts outside of the predetermined season
...
data.loc[unexpected_harrier_records, 'check'] = data.loc[unexpected_sex_records, 'check'] + 'unexpected age + sex combination, '
...
now = datetime.now()
data.to_csv('data_{}.csv'.format(now.strftime('%Y%m%dT%H%M')), index=False)
